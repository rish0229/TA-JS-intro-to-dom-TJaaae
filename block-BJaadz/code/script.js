...
allHrsArray.map((elm) => (elm.style.border = '1px solid tomato'));

// Change the background of all the hr to "antiquewhite" using for of loop.
for (let elm of allHrsArray) {
  elm.style.background = 'antiquewhite';
}
...
document.querySelector('hr.image').style.border = '2px solid purple';

// Hide the box number 17 (last box).
document.querySelector('div.archive').lastElementChild.style.display = 'none';

// Change the border of all the hr element from solid to dashed type
allHrsArray.map((elm) => (elm.style.border = '1px dashed tomato'));

// Create a paragraph element and store it in variable named 'para' using `createElement`
let para = document.createElement('p');
console.log(para);

// Change the inner text of para to "querySelector returns an element you can manipulate but querySelectorAll returns the collection of elements in array kind of structure."
para.innerText =
...
box2.append(
  (document.createElement('p').innerText = 'Append inserts as last child')
);
...
box3.prepend(
  (document.createElement('p').innerText = 'Prepend inserts as first child')
);
...
let oddBoxArr = Array.from(oddBox);
oddBoxArr.map((elm) => (elm.style.background = 'aliceblue'));
