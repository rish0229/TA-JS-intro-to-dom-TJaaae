...
for (let cardDetails of bookObject.books) {
  let cardDiv = document.createElement('div');
  cardDiv.classList.add('card');
  let imgElm = document.createElement('img');
  imgElm.setAttribute('src', cardDetails.image);
  cardDiv.append(imgElm);
  let titleElm = document.createElement('h2');
  titleElm.innerText = cardDetails.title;
  cardDiv.append(titleElm);
  let authorElm = document.createElement('p');
  authorElm.innerHTML = `<p>Author: ${cardDetails.author}</p>`;
  cardDiv.append(authorElm);
  let btnElm = document.createElement('button');
  btnElm.innerText = 'Buy Now';
  cardDiv.append(btnElm);
